...
if (typeof web3 !== 'undefined') {
    ...
    realEstateContractRegisterEvent = realEstateContractHook.Register({}, 'latest');
    realEstateContractTransferEvent = realEstateContractHook.Transfer({}, 'latest');
    updateStatus("\nRegistered for events from Contract");
    realEstateContractRegisterEvent.watch(
        function(error, result) {
            if (!error) {
                updateStatus("\nRegister Event: Owner: " + result.args._owner + " , landID: " + result.args._landID);
                $('#loaderID2').hide();
            } else {
                updateStatus("\nRegister Event Error: " + error);
            }
        }
    );
